...
#sfo_feb has already been created above
sfo_feb %>%
  summarise(n = n()) #print out 68
...
# type your code for Question 3 here, and Knit
  sfo_feb%>%
    ...
    slice(which.max(iqr_arr)) #get the record that contains max IQR
...
ggplot(data = nycflights, aes(x = speed, y = distance)) + geom_point()
...
nycflights <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <= 0, 'in time', 'delayed'))
nycflights %>%
  filter(dep_type == 'delayed') %>%
  summarise(arr = mean(arr_type == 'in time'))
```