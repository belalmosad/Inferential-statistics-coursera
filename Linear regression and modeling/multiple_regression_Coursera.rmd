...
summary(evals$score)
ggplot(data = evals) + geom_histogram(aes(x = score))
newSc <- evals %>%
    filter(score < 3)  # To evaluate number of students gave score less than 3.
```
...
#Using Score as response variable and gender as explanatory variable.

ggplot(data = evals) + geom_boxplot(aes(x = gender, y = score))
...
```{r jitter}
# type your code for the Exercise here, and Knit
ggplot(data = evals) + geom_jitter(aes(x = bty_avg, y = score))
...
```{r summary-model}
# type your code for the Exercise here, and Knit
linear_model <- lm(data = evals, score ~ bty_avg)
summary(linear_model)

## Equation: 3.881 + 0.0667 * bty_avg
```
...
residula <- resid(linear_model)
ggplot() + geom_histogram(aes(residula))
plot(evals$bty_avg, residula, ylab = "Residuals", xlab = "beauty avg")
abline(0,0)
```
...
## The answer: No, R value is small.
```
...
## We note that R value is becoming higher
