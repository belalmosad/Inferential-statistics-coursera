...
inference(data = nc, x = habit, y = weight, statistic = "mean", method = "theoretical", type = "ci")
...
inference(data = nc, y = weeks, method = "theoretical", type = "ci", statistic = "mean", conf_level = 0.99)
...
inference(data = nc, y = weeks, method = "theoretical", type = "ci", statistic = "mean", conf_level = 0.90)
# We note that the width is smaller
```
...
```{r gained-mature-ht}
# type your code for the Exercise here, and Knit
inference(data = nc, y = weight, x = mature, type = "ht", statistic = "mean", null = 0, alternative = "twosided", method = "theoretical")
# p-value = 0.8529
# We accept the null hypothesis, No difference.
```
...
nc %>%
  group_by(mature) %>%
  summarise(mx = max(mage), mn = min(mage))
```
...
## Question: The relation between the marriage state and the age of the mother.
The mean age of the married mothers and the mean age of the not married mothers.
Using hypothesis test to answer this question.
```{r pick-your-own}
# type your code for the Exercise here, and Knit
inference(data = nc, x = marital, y = mage, method = "theoretical", type = "ht", statistic = "mean",  
          null = 0, alternative = "twosided")
# p-value < 0.0001
# so we reject the null hypothesis. There is a difference.
```
