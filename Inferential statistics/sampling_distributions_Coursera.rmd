...
samp1 %>%
  summarise(samp_mean = mean(area)
            , samp_med = median(area)
            , samp_max = max(area)
            , samp_min = min(area)
            , samp_iqr = IQR(area)
...
# type your code for the Exercise here, and Run Document
sample_means50 %>%
  summarise(samp50_mean = mean(x_bar)
            , samp50_max = max(x_bar)
            , samp50_min = min(x_bar))
ggplot(data = sample_means50, aes(x = x_bar)) + geom_histogram(bandwidth = 250)
```
...
ames %>%
  sample_n(size = 50) %>%
  summarise(mean_rice = mean(price))
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means_price <- ames %>%
  rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>%
  summarise(x_plot = mean(price))
ggplot(data = sample_means_price, aes(x = x_plot)) + geom_histogram(binwidth = 500)
sample_means_price  %>%
  summarise(mm = mean(x_plot))
...
ames %>%
  sample_n(size = 15) %>%
  summarise(mean_price15 = mean(price))
...
sample_means15 <- ames %>%
  ...
  summarise(x_plot = mean(price))
ggplot(data = sample_means15, aes(x = x_plot)) + geom_histogram(binwidth = 250)

sample_means15 %>%
  summarise(mean = mean(x_plot))
```
...
sample_means150 <- ames %>%
  ...
  summarise(x_plot = mean(price))
ggplot(data = sample_means150, aes(x = x_plot)) + geom_histogram(binwidth = 250)
...
# use ggplot functions above for both of the variables: sample_means50 and sample_means150
#note the difference of the variability of both.
